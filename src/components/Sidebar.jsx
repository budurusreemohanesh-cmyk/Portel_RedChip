...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-[280px] h-full flex flex-col border-r border-primary/10 glass 
        transition-transform duration-300 ease-in-out shrink-0 text-left
        lg:translate-x-0 lg:static lg:h-screen lg:z-20 lg:w-[260px]
        ${isOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        {/* Logo */}
        <div className="h-20 flex items-center justify-between px-6 lg:px-8 border-b border-primary/10">
        ...
        {/* Navigation */}
        <nav className="flex-1 py-6 px-4 space-y-2 overflow-y-auto scroll-hide">
          {navItems.map((item) => {
